const e=document.querySelector("[meta-page-name]").getAttribute("meta-page-name"),t=Array.from(document.querySelectorAll(".header-nav-link"));t.forEach(t=>{let a=t.getAttribute("link-name");a===e?(console.log("Selected page: "+a),t.classList.add("is-active-link")):t.classList.remove("is-active-link")});const a=document.getElementById("headerOpenModalBtn"),n=document.getElementById("headerCloseModalBtn"),l=document.querySelector(".headerModal");function o(e){document.documentElement.dataset.theme=e}a.addEventListener("click",()=>{l.style.display="block"}),n.addEventListener("click",()=>{l.style.display="none"}),window.addEventListener("click",e=>{e.target===l&&(l.style.display="none")});const c=document.querySelectorAll(".theme-toggle");let d=localStorage.getItem("theme")||"light";o(d),c.forEach(e=>{e.classList.add("no-animation"),e.checked="dark"===d}),setTimeout(()=>{c.forEach(e=>{e.classList.remove("no-animation")})},1e3),c.forEach(e=>{e.addEventListener("change",()=>{let e="light"===d?"dark":"light";o(e),localStorage.setItem("theme",e),d=e})});// document.addEventListener('DOMContentLoaded', function () {
...
//         if (isDarkMode) {
//             mobileMenu.style.backgroundColor = 'var(--dark-modal-bg)';
//         } else {
//             mobileMenu.style.backgroundColor = 'var(--primary-brend)';
//         }
//     }
...
//     }
// });
//# sourceMappingURL=favorites.b22125c9.js.map
