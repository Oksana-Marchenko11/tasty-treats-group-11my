...
`;function K(){window.innerWidth<768?I.setPerPage(6):window.innerWidth<1280?I.setPerPage(8):I.setPerPage(9);let e=(0,r.default).getLs();I.getRecipes().then(t=>{let n="";t.results.forEach(t=>{let i=t._id in e?"favorite":"unfavorite";n+=`    
            <li class="item-cards">
...
<path opacity="0.5" fill-rule="evenodd" clip-rule="evenodd" d="M10.9939 4.70783C9.16115 2.5652 6.10493 1.98884 3.80863 3.95085C1.51234 5.91285 1.18905 9.19323 2.99234 11.5137C4.49166 13.443 9.02912 17.5121 10.5163 18.8291C10.6826 18.9764 10.7658 19.0501 10.8629 19.0791C10.9475 19.1043 11.0402 19.1043 11.1249 19.0791C11.2219 19.0501 11.3051 18.9764 11.4715 18.8291C12.9586 17.5121 17.4961 13.443 18.9954 11.5137C20.7987 9.19323 20.5149 5.89221 18.1791 3.95085C15.8434 2.00948 12.8266 2.5652 10.9939 4.70783Z" stroke="#F8F8F8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            ...
            `}),D.innerHTML=n,D.querySelectorAll(".add-fav-btn").forEach(e=>{e.addEventListener("click",function(e){console.log((0,r.default).togleFav(e.target.closest("button").dataset.recipeId))})}),D.querySelectorAll(".main-see-recipe").forEach(e=>{let t=i("6o7gM");e.addEventListener("click",e=>{t.default.open(e.target.dataset.recipeId)})});let a=document.querySelector(".pagination");(function(e,t,n,i){e=Number(e),t=Number(t);let a=window.innerWidth<768?2:3;if(console.log(e,t),t>1){n.innerHTML="",1===e?(n.innerHTML+='<button class="main-pag-btn" disabled><<</button>',n.innerHTML+='<button class="main-pag-btn" disabled><</button>'):(n.innerHTML+='<button class="main-pag-btn main-pag-btn-green" data-topage="1"><<</button>',n.innerHTML+=`<button class="main-pag-btn main-pag-btn-green" data-topage="${e-1}"><</button>`);for(let i=e-a;i<=e+a;i++)i>0&&i<=t&&(i===e?n.innerHTML+=`<button class="main-pag-btn main-pag-btn-green active"  data-topage="${i}">${i}</button>`:i===e+a||i===e-a?n.innerHTML+=`<button class="main-pag-btn" data-topage="${i}">...</button>`:n.innerHTML+=`<button class="main-pag-btn" data-topage="${i}">${i}</button>`);e===t?(n.innerHTML+='<button class="main-pag-btn" disabled>></button>',n.innerHTML+='<button class="main-pag-btn" disabled>>></button>'):(n.innerHTML+=`<button class="main-pag-btn main-pag-btn-green" data-topage="${e+1}">></button>`,n.innerHTML+=`<button class="main-pag-btn main-pag-btn-green" data-topage="${t}">>></button>`),n.querySelectorAll("[data-topage]").forEach(e=>{e.addEventListener("click",function(e){e.preventDefault(),I.setPage(Number(e.target.dataset.topage)),i()})})}})(t.page,t.totalPages,a,K),console.log(t)})}
//# sourceMappingURL=index.55622963.js.map
